...
namespace Phần_mềm_vàng_bạc_trang_sức.DAO_database_callback
{
    public class ServiceDAO
    {
        ...
        public bool AddService(string S_VoucherDate, int CustomerIDd, int StaffID, int Total, int TotalPrepayment, int TotalRemainingMoney)
        {
            ...
            string query = string.Format("INSERT INTO ServiceVoucher S_VoucherID, S_VoucherDate, CustomerID, StaffID, Total, TotalPrepayment, TotalRemainingMoney, Status) VALUES ((Select isnull(max(S_VoucherID)+1,1)ServiceVoucher), '{0}', '{1}', '{2}', '{3}', '{4}','{5}','Undone')", S_VoucherDate, CustomerIDd, StaffID, Total, TotalPrepayment, TotalRemainingMoney);
