...
namespace Phần_mềm_vàng_bạc_trang_sức.DAO_database_callback
{
    public class BillDAO
    {
        ...
        public bool AddBillDetails(int PID, int Amount, int SellUnitPrice,double Discount, double TAX, int Total)
        {
            ...
            string query = string.Format("INSERT INTO SalesSlip_Detail (SalesSlipID, PID, AMOUNT, SellUnitPrice, Discount, TAX, Total)  VALUES ((Select isnull(max(SalesSlipID),1) '{0}','{1}', '{2}', '{3}', '{4}','{5}',)", PID, Amount, SellUnitPrice, Discount, TAX, Total);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
        ...
        public bool Sell(int amount, int PID)
        {
            ...
            string query = string.Format("UPDATE Product SET Amount= (Select amount WHERE PID={0} - " + amount + " WHERE CustomerID={0}", PID);
            int result = DataProvider.Instance.ExecuteNonQuery(query);
            return result > 0;
        }
    }
